...
public class HexagonalGrid : MonoBehaviour
{
    ...
    [SerializeField] [Range(0, 10)] int size;
    const float SIZE = 5;
    float W = SIZE * 2;
    float H = SIZE * Mathf.Sqrt(3f);
    ...
    private void Start()
    {
        ...
        print($"points {points.Count}, lines {lines.Count}, poly {polygons.Count}");
    ...
    private void Update()
    {
        ...
        {
            polygons[auxCounter % polygons.Count].Draw(8);
            //print($"Points {polygons[auxCounter % polygons.Count].points.Count}, Lines {polygons[auxCounter % polygons.Count].lines.Count}");
    ...
    {
        Vector2 centre = new Vector2(W * row * .5f + (col % 2 == 0 ? 0 : .25f * W), H * col * .5f);
    ...
    private void drawPolygons()
    {
        ...
        Invoke("drawPolygons", refreshTime);
    }
